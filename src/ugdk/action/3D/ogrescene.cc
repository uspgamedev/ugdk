...
OgreScene::OgreScene() {
    scene_mgr_ = ugdk::graphic::threed::manager()->root()->createSceneManager("OctreeSceneManager");
...
OgreScene::~OgreScene() {
    if (z_order_ > 0)
        ugdk::graphic::threed::manager()->window()->removeViewport(z_order_);
    delete camera_;
    ugdk::graphic::threed::manager()->root()->destroySceneManager(scene_mgr_);
}

void OgreScene::OnPushed(int index) {
    z_order_ = index;
	//TODO: we could let user set with x/y/w/h of this viewport...
	viewport_ = ugdk::graphic::threed::manager()->window()->addViewport(camera_->camera(), z_order_, 0, 0, 1, 1);
